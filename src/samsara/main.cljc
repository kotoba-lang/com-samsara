(ns samsara.main
  "Kotodama WASM entrypoint for the Samsara clean-room actor (L4) — Clojure port.
  ...
  facts (`samsara.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "samsara")
(def tier "L4")
...
(def entity-specs
  [{:entity "Device"     :plural "devices"     :id-prefix "samsara_dev"
    :fields [:serial :model :status :firmware]            :required [:serial :model]
    :coerce {}                       :refs {}}
   {:entity "Telemetry"  :plural "telemetries" :id-prefix "samsara_tel"
    :fields [:deviceId :metric :value :recordedAt]        :required [:metric :value]
    :coerce {:value :float}          :refs {:deviceId "Device"}}
   {:entity "Gateway"    :plural "gateways"    :id-prefix "samsara_gat"
    :fields [:name :location :deviceCount]                :required [:name :location]
    :coerce {:deviceCount :int}      :refs {}}
   {:entity "Command"    :plural "commands"    :id-prefix "samsara_com"
    :fields [:deviceId :name :payload :status]            :required [:name :payload]
    :coerce {}                       :refs {:deviceId "Device"}}
   {:entity "Firmware"   :plural "firmwares"   :id-prefix "samsara_fir"
    :fields [:model :version :contentRef]                 :required [:model :version]
    :coerce {}                       :refs {}}
   {:entity "Alert"      :plural "alerts"      :id-prefix "samsara_ale"
    :fields [:deviceId :rule :severity]                   :required [:rule :severity]
    :coerce {}                       :refs {:deviceId "Device"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"samsara.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "samsara-compat" :tier tier :entities entities} 200])
...
(defn start! [] :samsara-compat/ready)
